...
| 公開日入力をダイアログ化 | `ui.prompt()` で毎回入力（既存記事は現在の公開日を初期値表示） |
| `createNewArticle()` を簡素化 | ページヘッダーへの記入案内を削除 |
| `deleteArticle()` でも `doc.getId()` を使用 | `findGitHubFileById()` で未公開チェック |
| `_publish()` で `existingFile` を先取り | 公開日ダイアログ前に GitHub 検索 → 既存公開日を初期値に |

#### メリット

- **ドキュメントに何も書かなくてよい** → 担当者が触れる箇所が減りミス防止
- **ページヘッダーの視認性問題が解消** → ヘッダー自体を使わない
- **ID が永続的かつ一意** → ドキュメント存在する限り変わらない
- **公開日がファイル名に入る** → GitHub `_posts/` と Drive フォルダのソート順が一致
